...
int Fl_Wayland_Dockable_Group_Driver::wld_target_box_class::handle(int event) {
  ...
  if (event == FL_DND_ENTER) {
    ...
    int x_when_docked, y_when_docked, w_when_docked, h_when_docked, r = 0;
#ifdef HAVE_XDG_TOPLEVEL_DRAG
    if (xdg_toplevel_drag) {
      ...
      if (*driver(dock)->place_holder_while_dragged()) {
        ...
        x_when_docked = (*driver(dock)->place_holder_while_dragged())->x();
        y_when_docked = (*driver(dock)->place_holder_while_dragged())->y();
        w_when_docked = (*driver(dock)->place_holder_while_dragged())->w();
        h_when_docked = (*driver(dock)->place_holder_while_dragged())->h();
      }
    } else
#endif
    {
      ...
      w_when_docked = dock->w();
      h_when_docked = dock->h();
    }
    // Replace 'this' by 'dock' in target_group keeping its position in the child array.
    // This is useful if target_group is an Fl_Tab by keeping the current active tab.
    dock->resize(x(), y(), w(), h());
    ...
    if (new_target_box_place) {
      ...
      insert_target_box_in_group(this, new_target_box_place, r,
               x_when_docked, y_when_docked, w_when_docked, h_when_docked);

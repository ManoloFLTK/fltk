...
/** The destructor of any docking target class must call this function.
 The destructor should be implemented as follows:
 \code
 virtual ~docking_target_class() {
...
void Fl_Dockable_Group::check_origin_target(Fl_Widget *target) {
  ...
  while (win) {
    if (!win->parent() && win->children() == 1 && win->child(0)->as_group() &&
        (win->child(0)->type() & Fl_Dockable_Group::DOCKABLE)) {
...
/** The \p handle(int) member function of any docking target class must call this function.
...
\endcode
 ...
 \param[out] processed a bool variable that will be assigned by the \p handle_target function. \p processed
 ...
 \param target transmit \p this.
 \param event the \p handle(int) function's single argument.
 \param inside_f a function returning \p true if and only if the mouse is in the target widget's docking area.
 ...
 \param dock_payload_f a function that adds the dockable to the target widget if that widget is an Fl_Group,
 or replaces the target by the dockable otherwise.
 \param undock_f NULL, or a function that performs operations that are necessary after FLTK has undocked the dockable
 and replaced it by an object from class Fl_Dockable_Group::Docking_Target_Box.
 \return use as return value of the \p handle(int) member function of the target class if \p handle_target set \p processed to true.
 */
...
//printf("handle_target: event=%d target=%p active_dockable_=%p \n",event,target,active_dockable_);
  ...
  switch (event) {
    ...
    case FL_DOCK_DRAG:
      ...
      // in the target when it's dragged and should not accept DOCK events
      if (target->as_group() && target->contains(active_dockable_)) return 0;
      if (acceptable_f && !acceptable_f(target, Fl_Dockable_Group::active_dockable_)) return 0;
      if (inside_f(target)) { // true if mouse is inside the target's docking area
        if (active_dockable_->active_target_ != target) {
          active_dockable_->active_target_ = target;
        }
    ...
    case FL_DOCK_RELEASE:
    {
      ...
      if (acceptable_f && !acceptable_f(target, dock)) return 0;
      ...
      break;
    case FL_UNDOCK:
      Fl_Dockable_Group::active_dockable_->origin_target_ = target;
      if (undock_f) undock_f(target, Fl_Dockable_Group::active_dockable_);
...
void Fl_Dockable_Group::Docking_Target_Box::default_state_f_(Fl_Widget *target, bool can_dock) {
...
int Fl_Dockable_Group::Docking_Target_Box::handle(int event) {
    bool processed;
    int retval = Fl_Dockable_Group::handle_target(processed, this, event,
                                                    inside_f, NULL, state_f_, dock_payload_f, NULL);
...
Fl_Window *Fl_Dockable_Group::make_draggable() {
  if (state() == DRAG)
    return window();
  origin_target_ = NULL;
  Fl_Window *win = driver_->undock();
  Fl::pushed(NULL);
  return win;
}
...
class resizable_draggable_win : public Fl_Window {
private:
  Fl_Cursor cursor_;
...
public:
  resizable_draggable_win(int w, int h, const char *l, Fl_Dockable_Group *dock) :
                          Fl_Window(w, h, l), cursor_(FL_CURSOR_DEFAULT) {
    box(FL_UP_BOX);
...
int resizable_draggable_win::handle(int event) {
  ...
  bool inside_dock = Fl::event_inside(child(0));
  if (event == FL_MOVE && !inside_dock && cursor_ == FL_CURSOR_DEFAULT) {
    ...
    if (Fl::event_x() < margin && Fl::event_y() < margin) {
      ...
      cursor_ = FL_CURSOR_NWSE;
      b = bottom_right;
    } else if (Fl::event_x() < margin) { cursor_ = FL_CURSOR_WE; b = left; }
    else if (Fl::event_y() < margin) { cursor_ = FL_CURSOR_NS; b = top; }
    else if (Fl::event_x() >= w() - margin) { cursor_ = FL_CURSOR_WE; b = right; }
    else if (Fl::event_y() >= h() - margin) { cursor_ = FL_CURSOR_NS; b = bottom; }
    else b = none;
    if (Fl_Dockable_Group_Driver::using_wayland &&
        (b == left || b == top || b == top_left || b == bottom_left || b == top_right)) {
      b = none;
    ...
    return 1;
  } else if (event == FL_DRAG && cursor_ != FL_CURSOR_DEFAULT) {
...
Fl_Window *Fl_Dockable_Group_Driver::undock() {
  // transform the dockable group into a draggable, borderless toplevel window
  ...
  Fl_Window *win = dockable_->resizable() ? new resizable_draggable_win(dockable_->w(), dockable_->h(),
                   label, dockable_) : new Fl_Window(dockable_->w(), dockable_->h(), label);
  win->end();
  win->add(dockable_);

...
void open_cb(Fl_Widget *, Fl_Native_Multiline_Text_Widget *box) {
  ...
  if ( fnfc.show() ) return;
  FILE *in = fl_fopen(fnfc.filename(), "r");
  fseek(in, 0, SEEK_END);
  long pos = ftell(in);
  rewind(in);
...
#define SUBWIN 0//1
#if SUBWIN
  Fl_Window *sub1 = new Fl_Window(20, 0, 450, 20,"sub1");
  single2 = new Fl_Native_Text_Widget(0, 0, 450, 20, "extra Fl_Native_Text_Widget");
  single2->value("single2");
  sub1->end();
#endif
  bool readonly = false;

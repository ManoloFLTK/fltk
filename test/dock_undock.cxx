  ...
  int handle(int event) override {
    ...
    if (event == FL_PUSH || event == FL_DRAG) {
      ...
      if (tab && value() == tab) {
        ...
        Fl_Dockable_Group *dock = (tab->as_group() && (tab->type() & Fl_Dockable_Group::DOCKABLE) ?
                                   (Fl_Dockable_Group*)tab : NULL);
        if (dock) {
...
private:
  ...
  static void target_state_f(Fl_Widget *target, bool can_dock) {
    ...
    if (can_dock && target_flex->children() == 1) {
      ...
      Fl_Dockable_Group::target_state_f_type *f = Fl_Dockable_Group::Docking_Target_Box::get_target_state_f();
      f(box, true);
      target_flex->redraw();
